...
OptionParser.new do |opts|
  ...
  opts.version      = "0.1b"
  opts.banner       = "STR2BIN: 文字列で記述された16進数\表\現をバイナリーデータに変換します\n"
                      " 起動例:\n"
                      "   STR2BIN -iINPUT.dat -oOUTPUT.dat\n"
                      " オプション:"
  opts.on('-i','--infile INFILE',   '（必須）入力元ファイル名を指定します') {|v| PGCONFIG[:i] = v}
  opts.on('-o','--outfile OUTFILE', '（必須）出力先ファイル名を指定します') {|v| PGCONFIG[:o] = v}
  opts.on('-l','--licence',         'ライセンス情報を表示します') do
    print <<EOD

Copyright (C)2008 Ayumu Aizawa All right reserved.

- このプログラムは無保証です
- このプログラムを著作者の許可無く改変することを禁じます
- このプログラムを著作者の許可無く頒布することを禁じます
- このプログラムを利用した事による損害について著作者は一切の責任を負いません
EOD
exit
  end
  opts.parse!(ARGV)
